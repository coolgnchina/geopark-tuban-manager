...
def api_rectify_progress():
    ...
    stats = db.session.query(
        ...
        db.func.count(Tuban.id).label('count')
    ).filter_by(is_deleted=0).group_by(Tuban.rectify_status).all()

    return jsonify([{
        'name': stat.rectify_status or '未分类',
        'value': stat.count
    } for stat in stats])
...
def api_monthly_trend():
    ...
    # 按月统计整改完成
    monthly_closed = db.session.query(
...
def api_park_ranking():
    ...
    stats = db.session.query(
        ...
        db.func.sum(db.case((Tuban.rectify_status == '未整改', 1), else_=0)).label('pending_count'),
        db.func.sum(db.case((Tuban.rectify_status == '整改中', 1), else_=0)).label('in_progress_count'),
        db.func.sum(db.case((Tuban.is_closed == '是', 1), else_=0)).label('closed_count')
    ).filter_by(is_deleted=0).group_by(Tuban.park_name).all()
...
def api_overdue_list():
    """超期图斑列表API"""
    overdue_tubans = Tuban.query.filter(
        Tuban.is_deleted == 0,
        Tuban.rectify_deadline < datetime.now().date(),
        Tuban.rectify_status.in_(['未整改', '整改中'])
    ).order_by(Tuban.rectify_deadline).all()
    ...
    return jsonify([{
        ...
        'rectify_deadline': tuban.rectify_deadline.isoformat() if tuban.rectify_deadline else None,
        'overdue_days': (datetime.now().date() - tuban.rectify_deadline).days if tuban.rectify_deadline else 0
...
def api_impact_analysis():
    ...
    stats = db.session.query(
        Tuban.impact_level,
        db.func.count(Tuban.id).label('count')
    ).filter_by(is_deleted=0).group_by(Tuban.impact_level).all()

    return jsonify([{
        'impact_level': stat.impact_level or '未分类',
        'count': stat.count
    } for stat in stats])