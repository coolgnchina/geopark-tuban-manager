...
def dictionaries():
    ...
    items = []
    if current_type:
        items = Dictionary.query.filter_by(dict_type=current_type).order_by(Dictionary.sort_order, Dictionary.id).all()
...
def add_dictionary():
    ...
    try:
        ...
        # 检查是否已存在
        existing = Dictionary.query.filter_by(dict_type=dict_type, dict_code=dict_code).first()
        if existing:
