...
def index():
    ...
    # 获取筛选选项数据
    # 功能区列表
    func_zones = db.session.query(Tuban.func_zone).filter(
        Tuban.is_deleted == 0,
        Tuban.func_zone.isnot(None)
    ).distinct().all()
    func_zones = [z[0] for z in func_zones if z[0]]

    # 问题类型列表
    problem_types = db.session.query(Tuban.problem_type).filter(
        Tuban.is_deleted == 0,
        Tuban.problem_type.isnot(None)
    ).distinct().all()
